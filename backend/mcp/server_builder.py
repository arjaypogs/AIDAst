...
Creates a low-level ``Server("aso-mcp")`` and registers its tools / resources.
Used by both the stdio entry point (``aso_mcp_server.py``) and the HTTP mount
...
def build_mcp_server(mcp_service) -> Server:
    ...
    ``mcp_service`` is the ``AsoMCPService`` instance used by handlers. It
